# E-TANK Replication Materials

This repository contains the standalone Dynare implementation used for the quantitative analysis in my thesis:

**Innovate or Compensate? Carbon-Tax Revenue Recycling in the Energy Transition**

## Files

* `e_tank_pf.mod` — standalone E-TANK perfect-foresight model.
* `README.md` — model and replication information.

The calibration and common initial steady state used in the thesis are embedded directly in the `.mod` file. No external calibration file is required.
...
The reference run reproduces the main quantitative results reported in the thesis:
...
| Aggregate resource-scaled welfare index |        5.801250 |
...
The model is solved under perfect foresight. The carbon-price path is exogenous, while carbon-tax revenues are allocated between clean R&D and household compensation. The quantitative experiments examine how this allocation affects the energy transition, output, and welfare across household types.
